...
def create_dataloader(dataset, batch_size, use_val=True, transform_dict=None, resize=None):
    if dataset == "GTSRB":
        ...
        # 确保 ./data 下有数据，或者开启 download=True
        train_dataset = GTSRB(root='./data', split='train', transform=transform_train, download=True)
        testset = GTSRB(root='./data', split='test', transform=transform_test, download=True)

        # 简单处理：如果开启 use_val，这里复用测试集作为验证集，或者你可以使用 SubsetRandomSampler 划分训练集
        trainloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=8)
        if use_val:
            valloader = torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=False, num_workers=8)
        else:
            valloader = None
