...
# from robustbench.utils import load_model # 暂时注释掉，避免依赖问题
...
def generate_adv_dataset(args, model):
    ...
    if args.dataset == "GTSRB":
        ...
        # 使用 Test 集生成对抗样本（或者划分的 Val 集）
        train_dataset = GTSRB(root='./data', split='test', transform=transform_test, download=True)
...
def layer_sharpness(args, model, epsilon=0.1):
    if "CIFAR" in args.dataset:
        ...
        # GTSRB 模型自带 normalize，这里使用 Identity 或保持空
        # 为了兼容代码结构，创建一个无操作的 Norm
    ...
    trainloader = torch.utils.data.DataLoader(generate_adv_dataset(args, deepcopy(model)), batch_size=512, shuffle=True,
                                              num_workers=0)
    origin_total = 0
    origin_loss = 0.0
    origin_acc = 0
    ...
    args.logger.info(
        "{:35}, Robust Loss: {:10.2f}, Robust Acc: {:10.2f}".format("Origin", origin_loss, origin_acc * 100))
    ...
    for layer_name, _ in model.named_parameters():
        if "weight" in layer_name and layer_name[:-len(".weight")] in layer_sharpness_dict.keys():
            ...
            args.logger.info("{:35}, MRC: {:10.2f}, Dropped Robust Acc: {:10.2f}".format(layer_name[:-len(".weight")],
                                                                                         max_loss - origin_loss,
                                                                                         (origin_acc - min_acc) * 100))
...
def main():
    ...
    # 添加 GTSRB，并设为默认
    parser.add_argument('--dataset', default="GTSRB", type=str, help="dataset",
                        choices=["CIFAR10", "CIFAR100", "TinyImageNet", "GTSRB"])
    # 默认类别数 43
    parser.add_argument('--num_classes', default=43, type=int, help='num classes')
    # 输入尺寸 32
    ...
    trainloader, _, testloader = create_dataloader(args.dataset, args.batch_size, use_val=False,
    ...
    # 选择鲁棒性评估函数
    if args.dataset == "GTSRB":
        evalulate_robustness = evaluate_gtsrb_robustness
    elif "CIFAR" in args.dataset:
    ...
    logger.info("==> Init train acc: {:.2f}%, test acc: {:.2f}%, robust acc: {:.2f}%".format(train_acc, test_acc,
                                                                                             test_robust_acc))
    ...
    logger.info(
        interpolation(args, logger, init_sd, deepcopy(model.state_dict()), model, testloader, criterion, model_save_dir,
                      evalulate_robustness))
