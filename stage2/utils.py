...
from torchvision.datasets import GTSRB  # Import GTSRB
...
def interpolation(args, logger, init_sd, ft_sd, model, dataloader, criterion, save_dir, eval_robustness_func):
    ...
    for alpha in alphas:
        ...
        logger.info(
            "==> Alpha: {:.2f}, test acc: {:.2f}%, test robust acc: {:.2f}%".format(alpha, test_acc, test_robust_acc))
...
# === 新增 GTSRB 鲁棒性评估 ===
def evaluate_gtsrb_robustness(args, model):
    transform_test = transforms.Compose([
        transforms.Resize((32, 32)),
        transforms.ToTensor(),
    ])

    dataset = GTSRB(root='./data', split='test', transform=transform_test, download=True)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size, num_workers=8)

    # 注意：GTSRB_ResNet18 模型内部已经包含了 Normalization，这里不需要额外包装 norm_layer
    # 如果是普通模型，可能需要添加 Normalization
